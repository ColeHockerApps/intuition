...
struct IntuAppPlayView: UIViewRepresentable {

    @EnvironmentObject private var orientation: IntuAppOrientationManager

    let startPoint: URL
    let launch: IntuAppLaunchStore
    let session: IntuAppSessionState
    let onReady: () -> Void
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: IntuAppLaunchStore
    private let orientation: IntuAppOrientationManager
    private let session: IntuAppSessionState
    ...
    init(
        startPoint: URL,
        launch: IntuAppLaunchStore,
        orientation: IntuAppOrientationManager,
        session: IntuAppSessionState,
    ...
    func begin() {
        ...
        session.setActivePoint(startPoint, base: launch.mainPoint)
        ...
        if webView === popupView {
            if let main = mainView,
               let next = action.request.url {
                ...
                session.setActivePoint(next, base: launch.mainPoint)
    ...
    private func handleFinish(in view: WKWebView) {
        ...
        guard let current = view.url else {
            ...
            session.setActivePoint(nil, base: launch.mainPoint)
            stopMarksJob()
            revealIfNeeded()
            return
        }
        ...
        session.setActivePoint(current, base: launch.mainPoint)
    ...
    private func handleFailure(in view: WKWebView) {
        ...
        session.setActivePoint(view.url, base: launch.mainPoint)
